...
class RSSFeedHandler:
    ...
    def __init__(self):
        self.feed_links = {
            ...
            'vb': ['https://www.vb.is/rss'],
            'ruv': ['https://www.ruv.is/rss/frettir']
        }
...
class NewsScraper(ABC):
    def __init__(self, debug_mode=False, source_name=None):
        ...
        self.output_dir = "outputs/news/json"
        self.links_dir = "outputs/news/links"
    ...
    def process_rss_articles(self, rss_data, target_date):
        ...
        if not self.debug_mode:
            print(
                f"\nFound {len(filtered_articles)} articles from {self.source_name} for {target_date_str}")
            article_iterator = tqdm(
                filtered_articles, desc="Processing articles", unit="article")
        else:
            article_iterator = filtered_articles
...
def main():
    ...
    print(f"\nStarting to process articles for {today}")
    ...
    output_file = os.path.join(
        "outputs", "news", "json", f"news_articles_{today}.json")
